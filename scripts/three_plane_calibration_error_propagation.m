...
function plot_main_group_results(data, group_name)
    ...
    plot_error_ellipse_2d(dx, dy, ...
        ...
        ['95% Origin-Position Error Ellipse: \Delta x-\Delta y, ', group_name]);

    figure;
    plot_error_ellipse_2d(dx, dz, ...
        '\Delta x / mm', '\Delta z / mm', ...
        ['95% Origin-Position Error Ellipse: \Delta x-\Delta z, ', group_name]);

    figure;
    plot_error_ellipse_2d(dy, dz, ...
        '\Delta y / mm', '\Delta z / mm', ...
        ['95% Origin-Position Error Ellipse: \Delta y-\Delta z, ', group_name]);
    ...
    ylabel('Count');
    title(['Estimated condition number after refined blind sequential RANSAC, ', group_name]);
    ...
    title(['Origin-position error versus condition number, ', group_name]);
    ...
    title(['Rotation error versus condition number, ', group_name]);
    ...
    ylabel('Origin-position / translation error / mm');
    title(['Origin-position error versus minimum plane angle, ', group_name]);
    ...
    grid on;
    ylabel('Origin-position / translation error / mm');
    title(['Origin-position error grouped by condition number, ', group_name]);
    xtickangle(20);
    ...
    grid on;
    ylabel('Rotation error / degree');
    title(['Rotation error grouped by condition number, ', group_name]);
    xtickangle(20);
    ...
    title(['Segmentation purity grouped by condition number, ', group_name]);
    xtickangle(20);
end
