...
%% ============================================================
%  Sequential RANSAC blind segmentation for three-plane LiDAR calibration
%
%  Goal:
%  Generate three randomly placed planar boards, mix all points into
%  one blind point cloud, automatically extract three planes using
%  sequential RANSAC, then perform coordinate-frame calibration.
%
%  This is closer to the real "blind scan" setting:
%  The algorithm does not know which point belongs to which board.
% =============================================================
...
outlier_magnitude = 0.08;   % 80 mm displacement along board normal

% Optional background clutter points.
% Set this to 0 first. Later you can increase it to test harder scenes.
...
for trial = 1:num_trials
    ...
    %% --------------------------------------------------------
    % 6. Match the three extracted planes to the three true planes
    %
    % In real data, plane order is arbitrary. For simulation evaluation,
    % we match extracted planes to ground truth by maximum normal similarity.
    %% --------------------------------------------------------
...
fprintf('\nValid blind-segmentation trials: %d / %d\n', num_valid, num_trials);
fprintf('Blind segmentation success rate: %.2f %%\n', success_rate);
...
min_purity_mean = nan(num_groups, 1);
...
for g = 1:num_groups
    ...
    if sample_count(g) > 0
        ...
        min_purity_mean(g) = mean(min_plane_purity(idx));
    end
end
...
disp(' ');
disp('================ Sequential RANSAC Blind Segmentation Group Summary ================');
...
disp(' ');
disp('================ Blind Segmentation Threshold Acceptance Summary ================');
...
fprintf('\n================ Overall Blind Segmentation Result ================\n');
...
ylabel('Count');
title('Estimated condition number after blind sequential RANSAC');
...
title('Origin error versus condition number after blind segmentation');
...
title('Rotation error versus condition number after blind segmentation');
...
title('Segmentation purity grouped by condition number');
xtickangle(20);
...
title('Percentage of blind scans in each condition group');
